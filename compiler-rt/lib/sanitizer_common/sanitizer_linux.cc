...
ThreadLister::Result ThreadLister::ListThreads(
  ...
  for (bool first_read = true;; first_read = false) {
    ...
    for (uptr begin = (uptr)buffer_.data(), end = begin + read; begin < end;) {
      ...
      // In Loongarch64, there are something wrong with internal_getdents
      // the first character of buffer.data is always wrong
      char *d_name_ptr = entry->d_name;
      if (*d_name_ptr < '0' || *d_name_ptr > '9') {
        ++d_name_ptr;
      }
      if (entry->d_ino && *d_name_ptr >= '0' && *d_name_ptr <= '9')
        threads->push_back(internal_atoll(d_name_ptr));
    }
