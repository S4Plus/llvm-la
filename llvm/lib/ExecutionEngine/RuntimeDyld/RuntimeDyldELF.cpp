...
void RuntimeDyldELF::resolveLoongArch64Relocation(const SectionEntry &Section,
  ...
  case ELF::R_LARCH_SOP_SL:
    ...
    break;
  case ELF::R_LARCH_32:
    support::ulittle32_t::ref{TargetPtr} = static_cast<uint32_t>(Value + Addend);
    break;
  case ELF::R_LARCH_64:
  ...
  case ELF::R_LARCH_SOP_POP_32_S_0_5_10_16_S2:
    ...
    //tmp1 >>= 2
    //get(inst)
    //inst=(inst & 0xfc0003e0)|((tmp1 & 0xffff) << 10)|((tmp1 & 0x1f0000) >> 16)
    //write(inst)
    tmp1 = ValuesStack.pop_back_val();
    tmp1 >>= 2;
    support::ulittle32_t::ref{TargetPtr} =
  ...
  case ELF::R_LARCH_SOP_POP_32_S_0_10_10_16_S2:
    ...
    //tmp1 >>= 2
    //get(inst)
    //inst=(inst & 0xfc000000)|((tmp1 & 0xffff) << 10)|((tmp1 & 0x3ff0000) >> 16)
    //write(inst)
    tmp1 = ValuesStack.pop_back_val();
    tmp1 >>= 2;
    support::ulittle32_t::ref{TargetPtr} =
...
void RuntimeDyldELF::resolveRelocation(const SectionEntry &Section,
  ...
  case Triple::loongarch64:
    ...
    break;
#if 0
  case Triple::loongarch: // XXX: now not support loongarch32
    resolveLoongArchRelocation(Section, Offset, (uint32_t)(Value & 0xffffffffL), Type,
                         (uint32_t)(Addend & 0xffffffffL));
    break;
#endif
  case Triple::ppc:
  ...
  if ((Arch == Triple::aarch64 || Arch == Triple::aarch64_be)) {
    ...
    if (IsSaved == true) {
      ...
      resolveRelocation(Sections[SectionID], Offset,
          reinterpret_cast<uint64_t>(TargetAddr), RelType, Addend);
    }
