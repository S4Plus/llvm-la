...
void OrcLoongArch32_Base::writeResolverCode(uint8_t *ResolverMem,
                                       JITReentryFn ReentryFn,
                                       void *CallbackMgr) {
// TODO: to be implemented
}

void OrcLoongArch32_Base::writeTrampolines(uint8_t *TrampolineMem,
                                      void *ResolverAddr,
                                      unsigned NumTrampolines) {
// TODO: to be implemented
}

Error OrcLoongArch32_Base::emitIndirectStubsBlock(IndirectStubsInfo &StubsInfo,
                                             unsigned MinStubs,
                                             void *InitialPtrVal) {
// TODO: to be implemented
  return Error::success();
...
void OrcLoongArch64::writeResolverCode(uint8_t *ResolverMem, JITReentryFn ReentryFn,
                                  void *CallbackMgr) {
  ...
  const uint32_t ResolverCode[] = {
      ...
      0x00000000,                    // 0x74: lu12i.w $a0,hi(callbackmgr)
      0x00000000,                    // 0x78: ori $a0,$a0,lo(callbackmgr)
      0x00000000,                    // 0x7c: lu32i.d $a0,higher(callbackmgr)
      0x00000000,                    // 0x80: lu52i.d $a0,$a0,highest(callbackmgr)
  ...
  const unsigned CallbackMgrAddrOffset = 0x74; // Callback manager addr lu12i.w

  memcpy(ResolverMem, ResolverCode, sizeof(ResolverCode));

  uint64_t CallMgrAddr = reinterpret_cast<uint64_t>(CallbackMgr);

  uint32_t CallMgrLU12i = 0x14000004 | ((CallMgrAddr << 32 >> 44) << 5);
  uint32_t CallMgrORi = 0x03800084 | ((CallMgrAddr & 0xFFF) << 10);
  uint32_t CallMgrLU32i = 0x16000004 | ((CallMgrAddr << 12 >> 44) << 5);
  uint32_t CallMgrLU52i = 0x03000084 | ((CallMgrAddr >> 52) << 10);

  memcpy(ResolverMem + CallbackMgrAddrOffset, &CallMgrLU12i,
         sizeof(CallMgrLU12i));
  memcpy(ResolverMem + (CallbackMgrAddrOffset + 4), &CallMgrORi,
         sizeof(CallMgrORi));
  memcpy(ResolverMem + (CallbackMgrAddrOffset + 8), &CallMgrLU32i,
         sizeof(CallMgrLU32i));
  memcpy(ResolverMem + (CallbackMgrAddrOffset + 12), &CallMgrLU52i,
         sizeof(CallMgrLU52i));

  uint64_t ReentryAddr = reinterpret_cast<uint64_t>(ReentryFn);

  uint32_t ReentryLU12i = 0x14000015 | ((ReentryAddr << 32 >> 44) << 5);
  uint32_t ReentryORi = 0x038002b5 | ((ReentryAddr & 0xFFF) << 10);
  uint32_t ReentryLU32i = 0x16000015 | ((ReentryAddr << 12 >> 44) << 5);
  uint32_t ReentryLU52i = 0x030002b5 | ((ReentryAddr >> 52) << 10);

  memcpy(ResolverMem + ReentryFnAddrOffset, &ReentryLU12i,
         sizeof(ReentryLU12i));
  memcpy(ResolverMem + (ReentryFnAddrOffset + 4), &ReentryORi,
         sizeof(ReentryORi));
  memcpy(ResolverMem + (ReentryFnAddrOffset + 8), &ReentryLU32i,
         sizeof(ReentryLU32i));
  memcpy(ResolverMem + (ReentryFnAddrOffset + 12), &ReentryLU52i,
         sizeof(ReentryLU52i));
}

void OrcLoongArch64::writeTrampolines(uint8_t *TrampolineMem, void *ResolverAddr,
                                 unsigned NumTrampolines) {

  uint32_t *Trampolines = reinterpret_cast<uint32_t *>(TrampolineMem);
  uint64_t ResolveAddr = reinterpret_cast<uint64_t>(ResolverAddr);

  uint64_t HiBits = ((ResolveAddr << 32 >> 44) << 5);
  uint64_t LoBits = ((ResolveAddr & 0xFFF) << 10);
  uint64_t HigherBits = ((ResolveAddr << 12 >> 44) << 5);
  uint64_t HighestBits = ((ResolveAddr >> 52) << 10);

  for (unsigned I = 0; I < NumTrampolines; ++I) {
    Trampolines[10 * I + 0] = 0x00150034;               // move $t8,$ra
    Trampolines[10 * I + 1] = 0x14000015 | HiBits;      // lu12i.w $r21,hi(ResolveAddr)
    Trampolines[10 * I + 2] = 0x038002b5 | LoBits;      // ori $r21,$r21,lo(ResolveAddr)
    Trampolines[10 * I + 3] = 0x16000015 | HigherBits;  // lu32i $r21,higher(ResolveAddr)
    Trampolines[10 * I + 4] = 0x030002b5 | HighestBits; // lu52i $r21,$r21,highest(ResolveAddr)
    Trampolines[10 * I + 5] = 0x4c0002a1;               // jirl $ra, $r21, 0
  }
}

Error OrcLoongArch64::emitIndirectStubsBlock(IndirectStubsInfo &StubsInfo,
                                        unsigned MinStubs,
                                        void *InitialPtrVal) {
  // Stub format is:
  //
  ...
  for (unsigned I = 0; I < NumStubs; ++I, PtrAddr += 8) {
    ...
    Stub[8 * I + 3] = 0x030002b5 | HighestBits; // lu52i.d $r21, $r21, highest(PtrAddr)
