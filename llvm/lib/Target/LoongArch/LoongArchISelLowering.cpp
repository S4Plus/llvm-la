...
const char *LoongArchTargetLowering::getTargetNodeName(unsigned Opcode) const {
  ...
  case LoongArchISD::VALL_ZERO:         return "LoongArchISD::VALL_ZERO";
  case LoongArchISD::VANY_ZERO:         return "LoongArchISD::VANY_ZERO";
  case LoongArchISD::VALL_NONZERO:      return "LoongArchISD::VALL_NONZERO";
  case LoongArchISD::VANY_NONZERO:      return "LoongArchISD::VANY_NONZERO";
  case LoongArchISD::VEXTRACT_SEXT_ELT: return "LoongArchISD::VEXTRACT_SEXT_ELT";
  case LoongArchISD::VEXTRACT_ZEXT_ELT: return "LoongArchISD::VEXTRACT_ZEXT_ELT";
  case LoongArchISD::VNOR:              return "LoongArchISD::VNOR";
  case LoongArchISD::VSHF:              return "LoongArchISD::VSHF";
  case LoongArchISD::SHF:               return "LoongArchISD::SHF";
  case LoongArchISD::VPACKEV:           return "LoongArchISD::VPACKEV";
  case LoongArchISD::VPACKOD:           return "LoongArchISD::VPACKOD";
  case LoongArchISD::VILVH:             return "LoongArchISD::VILVH";
  ...
  case LoongArchISD::VBSRL:             return "LoongArchISD::VBSRL";
  case LoongArchISD::VBSLL:             return "LoongArchISD::VBSLL";
  ...
  case LoongArchISD::XVBROADCAST:       return "LoongArchISD::XVBROADCAST";
...
addLSXIntType(MVT::SimpleValueType Ty, const TargetRegisterClass *RC) {
...
void LoongArchTargetLowering::
addLASXIntType(MVT::SimpleValueType Ty, const TargetRegisterClass *RC) {
  addRegisterClass(Ty, RC);
...
void LoongArchTargetLowering::
addLSXFloatType(MVT::SimpleValueType Ty, const TargetRegisterClass *RC) {
  addRegisterClass(Ty, RC);
...
void LoongArchTargetLowering::
addLASXFloatType(MVT::SimpleValueType Ty, const TargetRegisterClass *RC) {
  addRegisterClass(Ty, RC);
...
bool
LoongArchTargetLowering::allowsMisalignedMemoryAccesses(EVT VT,
                                                     unsigned,
                                                     unsigned,
                                                     bool *Fast) const {
  if (Fast)
    ...
    if (Op0Opcode == LoongArchISD::VEXTRACT_SEXT_ELT ||
      ...
      if (Mask) {
        ...
        if (Log2IfPositive > 0) {
          ...
          if ((Op0Opcode == LoongArchISD::VEXTRACT_ZEXT_ELT && Log2 >= ExtendTySize) ||
              Log2 == ExtendTySize) {
...
static SDValue performVSELECTCombine(SDNode *N, SelectionDAG &DAG) {
  ...
  if (TrueOpnd.getOperand(0) == Op1 &&
      TrueOpnd.getOperand(1) == Op2 &&
      FalseOpnd.getOperand(0) == Op2 &&
      FalseOpnd.getOperand(1) == Op1) {
    if (ISD::isSignedIntSetCC(CC)) {
      return DAG.getNode(LoongArchISD::VABSD, dl, N->getOperand(1).getValueType(),
          Op1, Op2,
          DAG.getTargetConstant(0, dl, MVT::i32));
    }else {
      return DAG.getNode(LoongArchISD::UVABSD, dl, N->getOperand(1).getValueType(),
          Op1, Op2,
          DAG.getTargetConstant(0, dl, MVT::i32));
    }
  }
...
static SDValue truncateVecElts(SDValue Op, SelectionDAG &DAG) {
  ...
  MVT ResEltTy = (ResTy == MVT::v2i64 || ResTy == MVT::v4i64) ? MVT::i64 : MVT::i32;
...
static SDValue lowerLASXVLDRIntr(SDValue Op, SelectionDAG &DAG, unsigned Intr,
  ...
  SDValue Load =  DAG.getLoad(ResTy, DL, ChainIn, Address, MachinePointerInfo(),
                            /* Alignment = */ 32);
    return DAG.getNode(LoongArchISD::XVBROADCAST, DL, DAG.getVTList(ResTy, MVT::Other), Load);
}
...
static SDValue lowerLSXVLDRIntr(SDValue Op, SelectionDAG &DAG, unsigned Intr,
  ...
  SDValue Load =  DAG.getLoad(ResTy, DL, ChainIn, Address, MachinePointerInfo(),
                            /* Alignment = */ 16);
    return DAG.getNode(LoongArchISD::VBROADCAST, DL, DAG.getVTList(ResTy, MVT::Other), Load);
}
...
static SDValue LowerFP_TO_SUINT(unsigned FPToSUI, unsigned ExtOpcode, SDValue Op, SelectionDAG &DAG) {
  ...
  if(ResTy.getScalarSizeInBits() == ViaTy.getScalarSizeInBits()) {
    ...
    Ops[0] = DAG.getNode(FPToSUI, DL, MVT::i32,
             DAG.getNode(ISD::EXTRACT_VECTOR_ELT, DL, MVT::f64, Op0, DAG.getConstant(0, DL, MVT::i64)));
    Ops[1] = DAG.getNode(FPToSUI, DL, MVT::i32,
             DAG.getNode(ISD::EXTRACT_VECTOR_ELT, DL, MVT::f64, Op0, DAG.getConstant(1, DL, MVT::i64)));
    Ops[2] = DAG.getNode(FPToSUI, DL, MVT::i32,
             DAG.getNode(ISD::EXTRACT_VECTOR_ELT, DL, MVT::f64, Op0, DAG.getConstant(2, DL, MVT::i64)));
    Ops[3] = DAG.getNode(FPToSUI, DL, MVT::i32,
             DAG.getNode(ISD::EXTRACT_VECTOR_ELT, DL, MVT::f64, Op0, DAG.getConstant(3, DL, MVT::i64)));
...
static SDValue lowerVECTOR_SHUFFLE_EXTRINS(SDValue Op, EVT ResTy,
                                       SmallVector<int, 16> Indices,
                                       SelectionDAG &DAG) {
  ...
  int EXTFromMask = -1;
  int EXTToMask = -1;
  if (Using1stVec == Size - 1 && Using2ndVec == 1) {
    for (int i = 0; i < Size; ++i) {
      if (Indices[i] >= Size) {
...
static SDValue lowerVECTOR_SHUFFLE_VBSHIFT(SDValue Op, EVT ResTy,
                                       SmallVector<int, 16> Indices,
                                       SelectionDAG &DAG) {
  assert((Indices.size() % 2) == 0);
  SDLoc DL(Op);
  ...
  unsigned Size = Indices.size();
  if (Using1stVec) {
    if (Indices[0] >= Size && Indices[0] < Size * 2) {  // vbsll
      int i = 0;
      for (i = 0; i < Size; ++i) {
        int Idx = Indices[i];
        if (Idx < 0 || Idx >= Size * 2)
      ...
      int SLLMask = i;
      return DAG.getNode(LoongArchISD::VBSLL, DL, MVT::v16i8, Op0,
                         DAG.getConstant(SLLMask, DL, MVT::i32));
    }
    else if (Indices[0] > 0 && Indices[0] < Size) { // vbsrl
      int i = 0;
      for (i = 0; i < Size; ++i) {
        int Idx = Indices[i];
        if (Idx < 0 || Idx >= Size * 2)
      ...
      for (int j = i + 1; j < Size; ++j) {
        int Idx = Indices[j];
        if (Idx < Size || Idx >= Size * 2)
          return SDValue();
      }
      int SRLMask = Indices[0];
      return DAG.getNode(LoongArchISD::VBSRL, DL, MVT::v16i8, Op0,
                         DAG.getConstant(SRLMask, DL, MVT::i32));
    }
  }

  if (Using2ndVec) {
    if (Indices[0] >= 0 && Indices[0] < Size) { // vbsll
      int i = 0;
      for (i = 0; i < Size; ++i) {
        int Idx = Indices[i];
        if (Idx < 0 || Idx >= Size * 2)
      ...
      return DAG.getNode(LoongArchISD::VBSRL, DL, MVT::v16i8, Op1,
                         DAG.getConstant(SRLMask, DL, MVT::i32));
    }
  }
...
//
// VILVL interleaves consecutive elements from the right (lowest-indexed) half of
// each vector.
//
...
static SDValue lowerVECTOR_SHUFFLE_XVPACKEV(SDValue Op, EVT ResTy,
  ...
  if (fitsRegularPattern<int>(Begin, 2, End, 0, 2)
    ...
    Xj = Op->getOperand(0);
           ...
           && fitsRegularPattern<int>(Begin + HalfSize, 2, End, Indices.size() + HalfSize, 2))
    Xj = Op->getOperand(1);
  else
    return SDValue();
  ...
  if (fitsRegularPattern<int>(Begin + 1, 2, End, 0, 2)
    ...
    Xk = Op->getOperand(0);
           ...
           && fitsRegularPattern<int>(Begin + 1 + HalfSize, 2, End, Indices.size() + HalfSize, 2))
    Xk = Op->getOperand(1);
  else
    return SDValue();
...
static SDValue lowerVECTOR_SHUFFLE_XVPACKOD(SDValue Op, EVT ResTy,
  ...
  if (fitsRegularPattern<int>(Begin, 2, End, 1, 2)
    ...
    Xj = Op->getOperand(0);
           ...
           && fitsRegularPattern<int>(Begin + HalfSize, 2, End, Indices.size() + HalfSize + 1, 2))
    Xj = Op->getOperand(1);
  else
    return SDValue();
  ...
  if (fitsRegularPattern<int>(Begin + 1, 2, End, 1, 2)
    ...
    Xk = Op->getOperand(0);
           ...
           && fitsRegularPattern<int>(Begin + 1 + HalfSize, 2, End, Indices.size() + HalfSize + 1, 2))
    Xk = Op->getOperand(1);
  else
    return SDValue();
...
static SDValue lowerVECTOR_SHUFFLE_XVREPLVE0(SDValue Op, EVT ResTy,
                                         SmallVector<int, 32> Indices,
                                         SelectionDAG &DAG) {
  assert((Indices.size() % 2) == 0);
  unsigned Size = Indices.size();
...
static SDValue lowerVECTOR_SHUFFLE_XVPICKEV(SDValue Op, EVT ResTy,
  ...
  if (fitsRegularPattern<int>(Begin, 1, LeftMid, 0, 2)
    ...
    Xj = Op->getOperand(0);
           ...
           && fitsRegularPattern<int>(Mid, 1, RightMid, Indices.size()+HalfSize, 2))
    Xj = Op->getOperand(1);
  else
    return SDValue();
  ...
  if (fitsRegularPattern<int>(LeftMid, 1, Mid, 0, 2)
    ...
    Xk = Op->getOperand(0);
           ...
           && fitsRegularPattern<int>(RightMid, 1, End, Indices.size()+HalfSize, 2))
    Xk = Op->getOperand(1);
  else
    return SDValue();
...
static SDValue lowerVECTOR_SHUFFLE_XVPICKOD(SDValue Op, EVT ResTy,
  ...
  if (fitsRegularPattern<int>(Begin, 1, LeftMid, 1, 2)
    ...
    Xj = Op->getOperand(0);
           ...
           && fitsRegularPattern<int>(Mid, 1, RightMid, Indices.size() + HalfSize + 1, 2))
    Xj = Op->getOperand(1);
  else
    return SDValue();
  ...
  if (fitsRegularPattern<int>(LeftMid, 1, Mid, 1, 2)
    ...
    Xk = Op->getOperand(0);
           ...
           && fitsRegularPattern<int>(RightMid, 1, End, Indices.size() + HalfSize + 1, 2))
    Xk = Op->getOperand(1);
  else
    return SDValue();
...
static SDValue lowerVECTOR_SHUFFLE_XVPERMIW(SDValue Op, EVT ResTy,
                                         SmallVector<int, 32> Indices,
                                         SelectionDAG &DAG) {
  MVT VT = Op.getSimpleValueType();
  SDValue Op1 = Op.getOperand(0);
  ...
  for (int i = 0; i < HalfSize; ++i) {
    int Idx = Indices[i];
    if ((Idx < 0) || (Idx + HalfSize != Indices[i + HalfSize])) {
      return SDValue();
    }
  }

  if (Indices[0] < HalfSize) {
    if (Indices[1] >= HalfSize || 
          Indices[2] < Size || Indices[2] >= Size + HalfSize || 
            Indices[3] < Size || Indices[3] >= Size + HalfSize) {
      return SDValue();
    }
  ...
  if (Indices[0] >= Size && Indices[0] < Size + HalfSize) {
    if (Indices[1] < Size || Indices[1] >= Size + HalfSize ||
          Indices[2] >= HalfSize || Indices[3] >= HalfSize) {
      return SDValue();
    }
...
static SDValue lowerVECTOR_SHUFFLE_XEXTRINS(SDValue Op, EVT ResTy,
                                         SmallVector<int, 32> Indices,
                                         SelectionDAG &DAG) {
  assert((Indices.size() % 2) == 0);
  unsigned Size = Indices.size();
...
MachineBasicBlock *
LoongArchTargetLowering::emitXINSERT_DF_VIDX(MachineInstr &MI,
  ...
  unsigned insertOp;
  insertOp = IsGPR64 ? LoongArch::XINSERT_FW_VIDX64_PSEUDO_POSTRA : LoongArch::XINSERT_FW_VIDX_PSEUDO_POSTRA;
  ...
  unsigned VecCopy = RegInfo.createVirtualRegister(RegInfo.getRegClass(SrcVecReg));
  unsigned LaneCopy = RegInfo.createVirtualRegister(RegInfo.getRegClass(LaneReg));
  unsigned ValCopy = RegInfo.createVirtualRegister(RegInfo.getRegClass(SrcValReg));

  const TargetRegisterClass *RC = IsGPR64 ? &LoongArch::GPR64RegClass : &LoongArch::GPR32RegClass;
  ...
  BuildMI(*BB, II, DL, TII->get(LoongArch::XVPICKVE2GR_W), Rj).addReg(Xj).addImm(0);
...
LoongArchTargetLowering::emitINSERT_H_VIDX(MachineInstr &MI,
  ...
  const TargetRegisterClass *RC = isGP64 ? &LoongArch::GPR64RegClass : &LoongArch::GPR32RegClass;
  unsigned RI = RegInfo.createVirtualRegister(RC);
...
LoongArchTargetLowering::emitXINSERT_B(MachineInstr &MI,
  ...
  unsigned tmp_Dst73 = RegInfo.createVirtualRegister(&LoongArch::LASX256BRegClass);
...
SDValue LoongArchTargetLowering::lowerINTRINSIC_WO_CHAIN(SDValue Op,
  ...
  case Intrinsic::loongarch_lsx_vmini_du:
    return DAG.getNode(ISD::UMIN, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerLSXSplatImm(Op, 2, DAG));
  ...
  case Intrinsic::loongarch_lsx_vsat_du: {
    ...
    switch (Intrinsic) {
    case Intrinsic::loongarch_lsx_vsat_b:
    case Intrinsic::loongarch_lsx_vsat_bu: Max = 7;  break;
    case Intrinsic::loongarch_lsx_vsat_h:
    case Intrinsic::loongarch_lsx_vsat_hu: Max = 15; break;
    case Intrinsic::loongarch_lsx_vsat_w:
    case Intrinsic::loongarch_lsx_vsat_wu: Max = 31; break;
    case Intrinsic::loongarch_lsx_vsat_d:
    case Intrinsic::loongarch_lsx_vsat_du: Max = 63; break;
...
//  case Intrinsic::loongarch_lsx_vshuf4i_d:
  ...
  case Intrinsic::loongarch_lsx_vsrari_d: {
    ...
    int64_t Max;
    switch (Intrinsic) {
    case Intrinsic::loongarch_lsx_vsrari_b: Max = 7; break;
    case Intrinsic::loongarch_lsx_vsrari_h: Max = 15; break;
    case Intrinsic::loongarch_lsx_vsrari_w: Max = 31; break;
    case Intrinsic::loongarch_lsx_vsrari_d: Max = 63; break;
    default: llvm_unreachable("Unmatched intrinsic");
    }
    int64_t Value = cast<ConstantSDNode>(Op->getOperand(2))->getSExtValue();
    if (Value < 0 || Value > Max)
  ...
  case Intrinsic::loongarch_lsx_vsrlri_d: {
    ...
    int64_t Max;
    switch (Intrinsic) {
    case Intrinsic::loongarch_lsx_vsrlri_b: Max = 7; break;
    case Intrinsic::loongarch_lsx_vsrlri_h: Max = 15; break;
    case Intrinsic::loongarch_lsx_vsrlri_w: Max = 31; break;
    case Intrinsic::loongarch_lsx_vsrlri_d: Max = 63; break;
    default: llvm_unreachable("Unmatched intrinsic");
    }
    int64_t Value = cast<ConstantSDNode>(Op->getOperand(2))->getSExtValue();
    if (Value < 0 || Value > Max)
  ...
  case Intrinsic::loongarch_lsx_vrotr_d:
    return DAG.getNode(LoongArchISD::VROR, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  ...
  case Intrinsic::loongarch_lsx_vrotri_d:
    return DAG.getNode(LoongArchISD::VRORI, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  }
}
...
SDValue LoongArchTargetLowering::lowerBUILD_VECTOR(SDValue Op,
  ...
  if (Node->isConstantSplat(SplatValue, SplatUndef, SplatBitSize,
                            HasAnyUndefs, 8) && SplatBitSize <= 64) {
    ...
    if((ResTy.is128BitVector() && !isLSXBySplatBitSize(SplatBitSize, ViaVecTy)) ||
       (ResTy.is256BitVector() && !isLASXBySplatBitSize(SplatBitSize, ViaVecTy)))
            return SDValue();
...
SDValue LoongArchTargetLowering::lowerFP_TO_UINT(SDValue Op, SelectionDAG &DAG) const {
  if(!Op->getValueType(0).isVector())
    return SDValue();
  return LowerFP_TO_SUINT(ISD::FP_TO_UINT, ISD::ZERO_EXTEND_VECTOR_INREG, Op, DAG);
}

SDValue LoongArchTargetLowering::lowerFP_TO_SINT(SDValue Op, SelectionDAG &DAG) const {
  if(Op->getValueType(0).isVector())
    return LowerFP_TO_SUINT(ISD::FP_TO_SINT, ISD::SIGN_EXTEND_VECTOR_INREG, Op, DAG);
...
static bool checkUndef(ArrayRef<int> Mask, unsigned Lo, unsigned Hi) {

  for (unsigned i = Lo, end = Hi; i != end; i++, Hi++)
...
static bool CheckRev(ArrayRef<int> Mask) {
  ...
  unsigned Num = Mask.size() - 1;
  for (unsigned i = 0; i < Mask.size(); i++, Num--)
    if (Mask[i] != Num)
      return false;
  return true;
}

static bool checkHalf(ArrayRef<int> Mask, unsigned Lo, unsigned Hi,
                      unsigned base) {
...
static SDValue lowerHalfHalf(const SDLoc &DL, MVT VT, SDValue Op1, SDValue Op2,
  ...
  unsigned HalfNum = Num / 2;

  if (Op1->isUndef() || Op2->isUndef() || Mask.size() > Num)
    return SDValue();
...
static bool checkHalfUndef(ArrayRef<int> Mask, unsigned Lo, unsigned Hi) {
...
static SDValue lowerHalfUndef(const SDLoc &DL, MVT VT, SDValue Op1, SDValue Op2,
  ...
  MVT VT2 = Op2.getSimpleValueType();
  unsigned Num1 = VT1.getVectorNumElements();
  unsigned Num2 = VT2.getVectorNumElements();
  SDValue Op;
...
SDValue LoongArchTargetLowering::lowerVECTOR_SHUFFLE(SDValue Op, 
  ...
  if (ResTy.is128BitVector()) {
    ...
    if ((Result = lowerVECTOR_SHUFFLE_PERMIW(Op, ResTy, Indices, DAG)))
      return Result;
    if ((Result = lowerVECTOR_SHUFFLE_EXTRINS(Op, ResTy, Indices, DAG)))
      return Result;
    if ((Result = lowerVECTOR_SHUFFLE_VBSHIFT(Op, ResTy, Indices, DAG)))
      return Result;
    ...
    if ((Result = lowerVECTOR_SHUFFLE_XVREPLVE0(Op, ResTy, Indices, DAG)))
      return Result;
  ...
  case 'f': // FPU or LSX register
    if (Subtarget.hasLSX() && type->isVectorTy() &&
      ...
      weight = CW_Register;
        ...
        cast<VectorType>(type)->getBitWidth() == 256)
      weight = CW_Register;
...
bool LoongArchTargetLowering::isLegalAddressingMode(const DataLayout &DL,
  ...
  //  4. reg1 + reg2
  // TODO: More checks should add for vector extension.
...
EVT LoongArchTargetLowering::getOptimalMemOpType(uint64_t Size, unsigned DstAlign,
                                            unsigned SrcAlign,
...
bool LoongArchTargetLowering::isFPImmLegal(const APFloat &Imm, EVT VT) const {
...
MachineBasicBlock *LoongArchTargetLowering::emitLSXCBranchPseudo(
  ...
  BuildMI(*FBB, FBB->end(), DL, TII->get(LoongArch::ADDI_W), RD1)
    .addReg(LoongArch::ZERO).addImm(0);
  ...
  BuildMI(*TBB, TBB->end(), DL, TII->get(LoongArch::ADDI_W), RD2)
    .addReg(LoongArch::ZERO).addImm(1);
...
LoongArchTargetLowering::emitCOPY_FW(MachineInstr &MI,
  ...
  if (Lane == 0) {
    unsigned Vj = Vk;
    BuildMI(*BB, MI, DL, TII->get(LoongArch::COPY), Fd).addReg(Vj, 0, LoongArch::sub_lo);
  } else {
    unsigned Vj = RegInfo.createVirtualRegister(&LoongArch::LSX128WRegClass);
    BuildMI(*BB, MI, DL, TII->get(LoongArch::VREPLVEI_W), Vj).addReg(Vk).addImm(Lane);
    BuildMI(*BB, MI, DL, TII->get(LoongArch::COPY), Fd).addReg(Vj, 0, LoongArch::sub_lo);
  }
...
LoongArchTargetLowering::emitCOPY_FD(MachineInstr &MI,
  ...
  if (Lane == 0)
    BuildMI(*BB, MI, DL, TII->get(LoongArch::COPY), Fd).addReg(Vk, 0, LoongArch::sub_64);
  else {
    unsigned Vj = RegInfo.createVirtualRegister(&LoongArch::LSX128DRegClass);
    assert(Lane == 1);

    BuildMI(*BB, MI, DL, TII->get(LoongArch::VREPLVEI_D), Vj).addReg(Vk).addImm(Lane);
    BuildMI(*BB, MI, DL, TII->get(LoongArch::COPY), Fd).addReg(Vj, 0, LoongArch::sub_64);
  }
...
LoongArchTargetLowering::emitXCOPY_FW(MachineInstr &MI,
  ...
  if (Lane == 0) {
    BuildMI(*BB, MI, DL, TII->get(LoongArch::COPY), Fd).addReg(Xj, 0, LoongArch::sub_lo);
  } else {
    BuildMI(*BB, MI, DL, TII->get(LoongArch::XVPICKVE2GR_WU), Rj).addReg(Xk).addImm(Lane);
...
LoongArchTargetLowering::emitXCOPY_FD(MachineInstr &MI,
  ...
  if(Lane == 0) {
    BuildMI(*BB, MI, DL, TII->get(LoongArch::COPY), Fd).addReg(Xk, 0, LoongArch::sub_64);
  } else {
    BuildMI(*BB, MI, DL, TII->get(LoongArch::XVPICKVE2GR_DU), Rj).addReg(Xk).addImm(Lane);
    BuildMI(*BB, MI, DL, TII->get(LoongArch::COPY), Fd)
        .addReg(Rj);
  }
...
MachineBasicBlock *LoongArchTargetLowering::emitCONCAT_VECTORS(
  ...
  BuildMI(*BB, MI, DL, TII->get(LoongArch::SUBREG_TO_REG), X0).addImm(0).addReg(SubReg1).addImm(LoongArch::sub_128);
  unsigned X1 = RegInfo.createVirtualRegister(RC);
  BuildMI(*BB, MI, DL, TII->get(LoongArch::SUBREG_TO_REG), X1).addImm(0).addReg(SubReg2).addImm(LoongArch::sub_128);
  BuildMI(*BB, MI, DL, TII->get(LoongArch::XVPERMI_Q), Xd).addReg(X0).addReg(X1).addImm(2);
...
LoongArchTargetLowering::emitXCOPY_FW_GPR(MachineInstr &MI,
  ...
  BuildMI(BB, DL, TII->get(LoongArch::ADDI_D), Rt1).addReg(LoongArch::ZERO_64).addImm(4);
  BuildMI(BB, DL, TII->get(LoongArch::BGE)).addReg(Lane).addReg(Rt1).addMBB(HBB);
  ...
  BuildMI(*LBB, LBB->end(), DL, TII->get(LoongArch::XVREPLVE_W_N), Xt1).addReg(Xs).addReg(Lane);
  BuildMI(*LBB, LBB->end(), DL, TII->get(LoongArch::COPY), Rf0).addReg(Xt1, 0, LoongArch::sub_lo);
  ...
  BuildMI(*HBB, HBB->end(), DL, TII->get(LoongArch::ADDI_D), Rt2).addReg(Lane).addImm(-4);
  BuildMI(*HBB, HBB->end(), DL, TII->get(LoongArch::XVPERMI_Q), Xt2).addReg(Xs).addReg(Xs).addImm(1);
  BuildMI(*HBB, HBB->end(), DL, TII->get(LoongArch::XVREPLVE_W_N), Xt3).addReg(Xt2).addReg(Rt2);
  BuildMI(*HBB, HBB->end(), DL, TII->get(LoongArch::COPY), Rf1).addReg(Xt3, 0, LoongArch::sub_lo);
...
MachineBasicBlock *
LoongArchTargetLowering::emitXINSERT_BH(MachineInstr &MI,
                                    MachineBasicBlock *BB,
                                    unsigned Size) const {
  const TargetInstrInfo *TII = Subtarget.getInstrInfo();
  MachineRegisterInfo &RegInfo = BB->getParent()->getRegInfo();
  ...
  if (Lane >= HalfSize) {
    ...
    BuildMI(*BB, MI, DL, TII->get(LoongArch::XVPERMI_Q), Xk).addReg(Xd_in).addReg(Xd_in).addImm(1);
    Imm = Lane - HalfSize;
  }

  unsigned Xk128 = RegInfo.createVirtualRegister(SubVecRC);
  unsigned Xd128 = RegInfo.createVirtualRegister(SubVecRC);
  BuildMI(*BB, MI, DL, TII->get(LoongArch::COPY), Xk128).addReg(Xk, 0, LoongArch::sub_128);
  BuildMI(*BB, MI, DL, TII->get(InsertOp), Xd128).addReg(Xk128).addReg(Fs).addImm(Imm);
  ...
  BuildMI(*BB, MI, DL, TII->get(LoongArch::SUBREG_TO_REG), Xd256).addImm(0).addReg(Xd128).addImm(LoongArch::sub_128);

  if (Lane >= HalfSize) {
    BuildMI(*BB, MI, DL, TII->get(LoongArch::XVPERMI_Q), Xd).addReg(Xd_in).addReg(Xd256).addImm(2);
  }
...
LoongArchTargetLowering::emitXINSERT_FW(MachineInstr &MI,
  ...
  BuildMI(*BB, MI, DL, TII->get(LoongArch::SUBREG_TO_REG), Xj)
      ...
      .addReg(Fs)
      .addImm(LoongArch::sub_lo);
  BuildMI(*BB, MI, DL, TII->get(LoongArch::XVPICKVE2GR_WU), Rj).addReg(Xj).addImm(0);
...
LoongArchTargetLowering::emitINSERT_FW(MachineInstr &MI,
  ...
  BuildMI(*BB, MI, DL, TII->get(LoongArch::SUBREG_TO_REG), Vj)
      ...
      .addReg(Fs)
      .addImm(LoongArch::sub_lo);
  BuildMI(*BB, MI, DL, TII->get(LoongArch::VPICKVE2GR_W), Rj).addReg(Vj).addImm(0);
...
LoongArchTargetLowering::emitINSERT_FD(MachineInstr &MI,
  ...
  BuildMI(*BB, MI, DL, TII->get(LoongArch::VPICKVE2GR_D), Rj).addReg(Vj).addImm(0);
...
LoongArchTargetLowering::emitFILL_FW(MachineInstr &MI,
  ...
  BuildMI(*BB, MI, DL, TII->get(LoongArch::INSERT_SUBREG), Vj2)
      ...
      .addReg(Fs)
      .addImm(LoongArch::sub_lo);
  BuildMI(*BB, MI, DL, TII->get(LoongArch::VREPLVEI_W), Vd).addReg(Vj2).addImm(0);
...
LoongArchTargetLowering::emitFILL_FD(MachineInstr &MI,
  ...
  BuildMI(*BB, MI, DL, TII->get(LoongArch::INSERT_SUBREG), Vj2)
      ...
      .addReg(Fs)
      .addImm(LoongArch::sub_64);
  BuildMI(*BB, MI, DL, TII->get(LoongArch::VREPLVEI_D), Vd).addReg(Vj2).addImm(0);
...
bool LoongArchTargetLowering::isFMAFasterThanFMulAndFAdd(EVT VT) const {
...
unsigned LoongArchTargetLowering::getRegisterByName(const char *RegName, EVT VT,
                                                    SelectionDAG &DAG) const {
