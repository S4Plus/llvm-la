...
LoongArchTargetMachine::LoongArchTargetMachine(const Target &T, const Triple &TT,
    ...
    : LLVMTargetMachine(T, computeDataLayout(TT, CPU, Options), TT,
      ...
      TLOF(llvm::make_unique<LoongArchTargetObjectFile>()),
...
LoongArchTargetMachine::getSubtargetImpl(const Function &F) const {
  ...
  if (!I) {
    ...
    I = llvm::make_unique<LoongArchSubtarget>(TargetTriple, CPU, FS, *this,
                                         Options.StackAlignmentOverride);
  }
  return I.get();
}

void LoongArchTargetMachine::resetSubtarget(MachineFunction *MF) {
  LLVM_DEBUG(dbgs() << "resetSubtarget\n");

  MF->setSubtarget(const_cast<LoongArchSubtarget *>(getSubtargetImpl(MF->getFunction())));
}
