...
void LoongArchFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  if (hasFP(MF)) {
    ...
    if (RegInfo.needsStackRealignment(MF)) {
      ...
      int Alignment = (int)MFI.getMaxAlignment();
...
int LoongArchFrameLowering::getFrameIndexReference(const MachineFunction &MF,
                                                   int FI,
...
void LoongArchFrameLowering::determineCalleeSaves(MachineFunction &MF,
  ...
  // If there is a variable sized object on the stack,
  // the estimation cannot account for it.
