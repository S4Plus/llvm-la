...
private:
  ...
  uint64_t visitELF(uint32_t Rel, RelocationRef R, uint64_t Value) {
    if (ObjToVisit.getBytesInAddress() == 8) { // 64-bit object file
      ...
      case Triple::loongarch64:
        return visitLoongArch64(Rel, R, Value);
      case Triple::ppc64le:
      case Triple::ppc64:
    ...
    case Triple::loongarch32:
      return visitLoongArch32(Rel, R, Value);
    case Triple::sparc:
  ...
  uint64_t visitLoongArch64(uint32_t Rel, RelocationRef R, uint64_t Value) {
    switch (Rel) {
    case ELF::R_LARCH_32:
      return (Value + getELFAddend(R)) & 0xFFFFFFFF;
    case ELF::R_LARCH_64:
  ...
  uint64_t visitLoongArch32(uint32_t Rel, RelocationRef R, uint64_t Value) {
    switch (Rel) {
    case ELF::R_LARCH_32:
      return Value; // FIXME: maybe need adjust value
    }
    HasError = true;
    return 0;
  }
