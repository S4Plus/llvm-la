...
ABIArgInfo LoongArchABIInfo::classifyArgumentType(QualType Ty, bool IsFixed,
  ...
  if(Ty->isVectorType() && (((getContext().getTypeSize(Ty) == 128) && (getTarget().hasFeature("lsx")))
                            || ((getContext().getTypeSize(Ty) == 256) && getTarget().hasFeature("lasx"))))
    return ABIArgInfo::getDirect();
...
public:
  LoongArchTargetCodeGenInfo(CodeGen::CodeGenTypes &CGT, unsigned GRLen,
                         unsigned FRLen)
      : TargetCodeGenInfo(new LoongArchABIInfo(CGT, GRLen, FRLen)) {}
