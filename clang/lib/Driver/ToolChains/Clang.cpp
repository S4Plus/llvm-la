...
static void CollectArgsForIntegratedAssembler(Compilation &C,
  ...
  const char *LoongArchTargetFeature = nullptr;
  for (const Arg *A :
    ...
    for (StringRef Value : A->getValues()) {
      ...
      case llvm::Triple::mips64el:
        ...
        LoongArchTargetFeature = llvm::StringSwitch<const char *>(Value)
                                .Case("-loongarch", "+loongarch")
                                .Default(nullptr);
        if (LoongArchTargetFeature)
          continue;
      }
  ...
  if (LoongArchTargetFeature != nullptr) {
    CmdArgs.push_back("-target-feature");
    CmdArgs.push_back(LoongArchTargetFeature);
  }
