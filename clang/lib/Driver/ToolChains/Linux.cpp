...
static std::string getMultiarchTriple(const Driver &D,
  ...
  bool IsLoongArchN32Abi = TargetTriple.getEnvironment() == llvm::Triple::GNUABIN32;
  ...
  case llvm::Triple::loongarch64: {
    std::string MT = std::string("loongarch64") +
                     "-linux-" + (IsLoongArchN32Abi ? "gnuabin32" : "gnuabi64");
    if (D.getVFS().exists(SysRoot + "/lib/" + MT))
      return MT;
    if (D.getVFS().exists(SysRoot + "/lib/loongarch64-linux-gnu"))
...
void Linux::AddClangSystemIncludeArgs(const ArgList &DriverArgs,
  ...
  const StringRef LoongArchMultiarchIncludeDirs[] = {
      "/usr/include/loongarch-linux-gnu"};
  ...
  case llvm::Triple::loongarch64:
    ...
    break;
  case llvm::Triple::loongarch32:
    MultiarchIncludeDirs = LoongArchMultiarchIncludeDirs;
    break;
  default:
    break;
  }
...
SanitizerMask Linux::getSupportedSanitizers() const {
  ...
  if (IsX86_64 || IsMIPS64 || IsAArch64 || IsX86 || IsArmArch || IsPowerPC64 || IsLoongArch64)
    Res |= SanitizerKind::Leak;
  if (IsX86_64 || IsMIPS64 || IsAArch64 || IsPowerPC64 || IsLoongArch64)
    Res |= SanitizerKind::Thread;
  if (IsX86_64)
    Res |= SanitizerKind::KernelMemory;
  if (IsX86_64 || IsMIPS64 || IsLoongArch64)
