...
struct RelStack {
  ...
  void push_back(uint64_t e) {
    if (top < REL_STACK_MAX_SIZE) {
      ...
      report_fatal_error("stack is overflow, top = " + top);
    }
  }
  ...
  uint64_t pop_back_val() {
    ...
    if (top >= 0) {
      ...
      report_fatal_error("stack is empty, top = " + top);
    }
    return e;
  }
};
...
template <class ELFT>
void LoongArch<ELFT>::writePlt(uint8_t *buf, uint64_t gotPltEntryAddr,
...
template <class ELFT>
void LoongArch<ELFT>::relocateOne(uint8_t *loc, RelType type,
                                  uint64_t val) const {
  switch (type) {
  ...
  case R_LARCH_SOP_POP_32_S_10_5: {
    uint64_t opr1 = relStack.pop_back_val();
    checkInt(loc, static_cast<int64_t>(opr1), 5, type);
  ...
  case R_LARCH_SOP_POP_32_S_10_12: {
    uint64_t opr1 = relStack.pop_back_val();
    checkInt(loc, static_cast<int64_t>(opr1), 12, type);
  ...
  case R_LARCH_SOP_POP_32_S_10_16: {
    uint64_t opr1 = relStack.pop_back_val();
    checkInt(loc, static_cast<int64_t>(opr1), 16, type);
  ...
  case R_LARCH_SOP_POP_32_S_10_16_S2: {
    ...
    checkInt(loc, static_cast<int64_t>(opr1), 18, type);
    checkAlignment(loc, opr1, 4, type);
  ...
  case R_LARCH_SOP_POP_32_S_5_20: {
    uint64_t opr1 = relStack.pop_back_val();
    checkInt(loc, static_cast<int64_t>(opr1), 20, type);
  ...
  case R_LARCH_SOP_POP_32_S_0_5_10_16_S2: {
    uint64_t opr1 = relStack.pop_back_val();
    checkInt(loc, static_cast<int64_t>(opr1), 23, type);
    checkAlignment(loc, opr1, 4, type);
  ...
  case R_LARCH_SOP_POP_32_S_0_10_10_16_S2: {
    uint64_t opr1 = relStack.pop_back_val();
    checkInt(loc, static_cast<int64_t>(opr1), 28, type);
    checkAlignment(loc, opr1, 4, type);
...
template <class ELFT>
RelExpr LoongArch<ELFT>::adjustRelaxExpr(RelType type, const uint8_t *data,
                                         RelExpr expr) const {
  return expr;
}

template <class ELFT>
void LoongArch<ELFT>::relaxTlsGdToLe(uint8_t *loc, RelType type,
                                     uint64_t val) const {
  return;
}

template <class ELFT>
void LoongArch<ELFT>::relaxTlsGdToIe(uint8_t *loc, RelType type,
                                     uint64_t val) const {
  return;
}

template <class ELFT>
void LoongArch<ELFT>::relaxTlsIeToLe(uint8_t *loc, RelType type,
                                     uint64_t val) const {
  return;
}
