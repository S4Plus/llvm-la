...
handleTlsRelocation(RelType Type, Symbol &Sym, InputSectionBase &C,
  ...
  if (isRelExprOneOf<R_TLSDESC, R_AARCH64_TLSDESC_PAGE, R_TLSDESC_CALL,
    ...
    if (Config->Shared || Config->EMachine == EM_LOONGARCH) {
      if (In.Got->addDynTlsEntry(Sym)) {
        uint64_t Off = In.Got->getGlobalDynOffset(Sym);
        if (isLocalInExecutable)
          In.Got->Relocations.push_back({R_ADDEND, Target->TlsModuleIndexRel, Off, 1, &Sym});
        else
...
static bool isStaticLinkTimeConstant(RelExpr E, RelType Type, const Symbol &Sym,
  ...
  if (isRelExprOneOf<R_GOT_FROM_END, R_GOT_OFF, R_HEXAGON_GOT, R_TLSLD_GOT_OFF,
                     ...
                     R_TLSLD_HINT, R_TLSIE_HINT, R_LARCH_ABS, R_LARCH_GOTREL>(E))
    return true;
...
static void processRelocAux(InputSectionBase &Sec, RelExpr Expr, RelType Type,
  ...
  if (!CanWrite && (Config->Pic && !isRelExpr(Expr))) {
    // R_LARCH_64/R_LARCH_32 is generated into .eh_frame section on LoongArch,
    // they are not dynamic relocation and are relocated later in link time.
    // It is a misreport here.
    if (Type == R_LARCH_64 || Type == R_LARCH_32)
      return;
    error(
